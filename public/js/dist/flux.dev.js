...
(function ($) {
  flux.slider = function (elem, opts) {
    ...
    this.element = $(elem); // Make a list of all available transitions
    ...
    }, opts); // Set the height/width if given [EXPERIMENTAL!]

    this.height = this.options.height ? this.options.height : null;
    this.width = this.options.width ? this.options.width : null; // Filter out non compatible transitions
    ...
    this.surface = $('<div class="surface" style="position: relative"></div>').appendTo(this.container); // Listen for click events as we may want to follow a link
    ...
    this.imageContainer = $('<div class="images loading"></div>').css({
      ...
      'overflow': 'hidden',
      'min-height': '100px'
    }).appendTo(this.surface); // If the height/width is already set then resize the container
    ...
    this.image1 = $('<div class="image1" style="height: 15%; width: 100%"></div>').appendTo(this.imageContainer);
    this.image2 = $('<div class="image2" style="height: 15%; width: 100%"></div>').appendTo(this.imageContainer);
    ...
    this.element.find('img, a img').each(function (index, found_img) {
      var imgClone = found_img.cloneNode(false),
          link = $(found_img).parent(); // If this img is directly inside a link then save the link for later use

      if (link.is('a')) $(imgClone).data('href', link.attr('href'));

      _this.images.push(imgClone); // Remove the images from the DOM
    ...
    if (this.options.autoplay) this.start(); // Handle swipes
  ...
  flux.slider.prototype = {
    ...
    isPlaying: function isPlaying() {
      return this.playing; //return this.interval != null;
    },
    next: function next(trans, opts) {
      opts = opts || {};
      opts.direction = 'left';
      this.showImage(this.currentImageIndex + 1, trans, opts);
    },
    prev: function prev(trans, opts) {
      opts = opts || {};
      opts.direction = 'right';
      this.showImage(this.currentImageIndex - 1, trans, opts);
    },
    showImage: function showImage(index, trans, opts) {
      this.setNextIndex(index); // Temporarily stop the transition interval
      //clearInterval(this.interval);
      //this.interval = null;
    ...
    finishedLoading: function finishedLoading() {
      ...
      this.imageContainer.removeClass('loading'); // Should we setup a pagination view?
    ...
    transition: function transition(_transition, opts) {
      // Allow a transition to be picked from ALL available transitions (not just the reduced set)
      if (_transition == undefined || !flux.transitions[_transition]) {
        // Pick a transition at random from the (possibly reduced set of) transitions
        var index = Math.floor(Math.random() * this.options.transitions.length);
        _transition = this.options.transitions[index];
      }

      var tran = null;

      try {
        tran = new flux.transitions[_transition](this, $.extend(this.options[_transition] ? this.options[_transition] : {}, opts));
      } catch (e) {
    ...
    setNextIndex: function setNextIndex(nextIndex) {
  ...
  flux.browser = {
    init: function init() {
      ...
      var div = document.createElement('div'),
          ...
          domPrefixes = ['Webkit', 'Moz', 'O', 'Ms']; // Does the current browser support CSS Transitions?

      if (window.Modernizr && Modernizr.csstransitions !== undefined) flux.browser.supportsTransitions = Modernizr.csstransitions;else {
    ...
    supportsCSSProperty: function supportsCSSProperty(prop) {
      var div = document.createElement('div'),
          prefixes = ['-webkit', '-moz', '-o', '-ms'],
    ...
    rotateX: function rotateX(deg) {
      return flux.browser.rotate('x', deg);
    },
    rotateY: function rotateY(deg) {
      return flux.browser.rotate('y', deg);
    },
    rotateZ: function rotateZ(deg) {
      return flux.browser.rotate('z', deg);
    },
    rotate: function rotate(axis, deg) {
      if (!axis in {
        'x': '',
        'y': '',
        'z': ''
      ...
      if (flux.browser.supports3d) return 'rotate3d(' + (axis == 'x' ? '1' : '0') + ', ' + (axis == 'y' ? '1' : '0') + ', ' + (axis == 'z' ? '1' : '0') + ', ' + deg + 'deg)';else {
        if (axis == 'z') return 'rotate(' + deg + 'deg)';else return '';
      }
    }
  };
  $(function () {
...
(function ($) {
  ...
  flux.transition = function (fluxslider, opts) {
    this.options = $.extend({
      requires3d: false,
      after: function after() {// Default callback for after the transition has completed
      }
    }, opts);
    this.slider = fluxslider; // We need to ensure transitions degrade gracefully if the transition is unsupported or not loaded
  ...
  flux.transition.prototype = {
    ...
    run: function run() {
      var _this = this; // do something


      if (this.options.setup !== undefined) this.options.setup.call(this); // Remove the background image from the top image

      this.slider.image1.css({
        'background-image': 'none'
      });
      this.slider.imageContainer.css('overflow', this.options.requires3d ? 'visible' : 'hidden'); // For some of the 3D effects using Zepto we need to delay the transitions for some reason
    ...
    finished: function finished() {
      ...
      this.slider.setupImages(); // Trigger an event to signal the end of a transition

      this.slider.element.trigger('fluxTransitionEnd', {
        currentImage: this.slider.getImage(this.slider.currentImageIndex)
      });
    },
    fallbackSetup: function fallbackSetup() {},
    fallbackExecute: function fallbackExecute() {
      this.finished();
    }
  };
  flux.transitions = {}; // Flux grid transition
  ...
  flux.transition_grid = function (fluxslider, opts) {
    return new flux.transition(fluxslider, $.extend({
      ...
      execute: function execute() {
        ...
        this.slider.image2.hide(); // Get notified when the last transition has completed
      ...
      renderTile: function renderTile(elem, colIndex, rowIndex, colWidth, rowHeight, leftOffset, topOffset) {}
    }, opts));
  };
})(window.jQuery || window.Zepto);
  ...
  flux.transitions.bars = function (fluxslider, opts) {
    return new flux.transition_grid(fluxslider, $.extend({
      ...
      execute: function execute() {
        ...
        var bars = this.slider.image1.find('div.tile'); // Get notified when the last transition has completed
  ...
  flux.transitions.blocks = function (fluxslider, opts) {
    return new flux.transition_grid(fluxslider, $.extend({
      ...
      execute: function execute() {
        ...
        var blocks = this.slider.image1.find('div.tile'); // Get notified when the last transition has completed
  ...
  flux.transitions.concentric = function (fluxslider, opts) {
    return new flux.transition(fluxslider, $.extend({
      ...
      execute: function execute() {
        ...
        var blocks = this.slider.image1.find('div.block'); // Get notified when the last transition has completed
  ...
  flux.transitions.cube = function (fluxslider, opts) {
    return new flux.transition(fluxslider, $.extend({
      ...
      setup: function setup() {
        ...
        var height = this.slider.image1.height(); // Setup the container to allow 3D perspective
      ...
      transitionStrings: function transitionStrings(direction, elem) {
        var width = this.slider.image1.width();
        var height = this.slider.image1.height(); // Define the various transforms that are required to perform various cube rotations
    ...
    return new flux.transition_grid(fluxslider, $.extend({
      ...
      renderTile: function renderTile(elem, colIndex, rowIndex, colWidth, rowHeight, leftOffset, topOffset) {
        ...
        var tile2 = $(tile.get(0).cloneNode(false)).css({
          'background-image': this.slider.image2.css('background-image') //'z-index': 190 // Removed to make compatible with FF10 (Chrome bug seems to have been fixed)
  ...
  flux.transitions.swipe = function (fluxslider, opts) {
    return new flux.transition(fluxslider, $.extend({
      ...
      execute: function execute() {
        //return;
        var _this = this,
            img = this.slider.image1.find('div'); // Get notified when the last transition has completed
...
(function ($) {
  flux.transitions.dissolve = function (fluxslider, opts) {
    return new flux.transition(fluxslider, $.extend({
      ...
      execute: function execute() {
        var _this = this,
            img = this.slider.image1.find('div.image'); // Get notified when the last transition has completed
